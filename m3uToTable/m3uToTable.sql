...
-- 获取每首歌曲已打标签数： 
select song.*, tmptable.cnt from song, (select song_id, count(1) as cnt from song_tag group by song_id) AS tmptable where song.id = tmptable.song_id order by tmptable.cnt desc
